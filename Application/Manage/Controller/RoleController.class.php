...
class RoleController extends AdminController {
    ...
    public function add() {
        ...
        $res_add = $this->role_mod->addRole($param);
        if (102 == $res_add['code']) {
            $this->error($res_add['status'], U('/Manage/Role/index/p/' . $this->p));
        } elseif (200 == $res_add['code']) {
            $this->success($res_add['status'], U('/Manage/Role/index/p/' . $this->p));
        } else {
            $this->error($res_add['status'], U('/Manage/Role/index/p/' . $this->p));
        }
    }
